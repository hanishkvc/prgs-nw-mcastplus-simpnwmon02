...
DoneModes tracks has to what and all phases of the transfer are already
...
Currently the autoadjust overwrites the runmodes, even if it is explicitly
specified in the commandline, if donemodes was part of the context file being
loaded. Will have to change the logic such that this auto adjusting only occurs
if the user has explicitly specified from the commandline that runmodes should
be autoadjusted.
...
# runmodes = 65536 means autoadjust runmodes from donemodes, if no donemodes in
# loaded context, runmodes will default to 7.
...
NOTE: The below wont work currently unless one removes the "#DoneModes:" line
from the saved_contextfile
...
Client> ./simpnwmon02 --maddr 230.0.0.1 --local 0 10.0.2.11 --file /path/to/datafile --bcast 10.0.2.255 --context /path/to/saved_contextfile
...
Client> ./simpnwmon02 --maddr 230.0.0.1 --local 0 10.0.2.11 --file /dev/null --bcast 10.0.2.255 --context /path/to/saved_contextfile --rumodes 7

If the client was force quit in the middle of unicast phase, then to resume run the below
Client> ./simpnwmon02 --maddr 230.0.0.1 --local 0 10.0.2.11 --file /dev/null --bcast 10.0.2.255 --context /path/to/saved_contextfile --rumodes 6
